...
void ParticleSystem::respawnParticle(Particle& p, glm::vec3 origin) {
    p.Position = origin;
    float horizontalSpeed = glm::linearRand(1.5f, 4.0f);
    float angle = glm::linearRand(0.0f, 2.0f * 3.14159f); // ángulo radial

    glm::vec3 horizontalDir = glm::vec3(cos(angle), 0.0f, sin(angle)) * horizontalSpeed;
    float verticalSpeed = glm::linearRand(5.0f, 10.0f);  // más impulso hacia arriba

    p.Position = origin;
    p.Velocity = horizontalDir + glm::vec3(0.0f, verticalSpeed, 0.0f);
